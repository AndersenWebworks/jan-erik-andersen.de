...
(function () {
  ...
  function getStatusText(nodeId) {
    if (nodeId === '_contact') return 'Contact draft is ready.';
    var node = tree && tree[nodeId];
    if (!node) return '';
    if (node.type === 'result') return 'Result: ' + (node.title || 'Recommendation');
    if (node.type === 'info') return 'Note: ' + (node.text || '').replace(/\s+/g, ' ').slice(0, 90);
    var step = history.length + 1;
    var total = history.length + (nodeDepths[nodeId] || 1);
    return 'Step ' + step + ' of ' + total + ': ' + (node.text || 'New question');
  }
  ...
  function buildResult(node, nodeId) {
    ...
    h += '<span class="funnel-action-icon" aria-hidden="true">' + ICONS['refresh-cw'] + '</span> Start over</button>';
    ...
    h += '<span class="funnel-action-icon" aria-hidden="true">' + ICONS['external-link'] + '</span> Read more about me</button>';
    h += '</div>';
    return h;
  }
